...
There is one detailed presentation, with no Light/High switch. The continuous camera route, main-stage band, 520-person Bermel crowd and 1,214-person arena remain:
...
- Hero and standard people currently load the original self-contained profiles. The [cross-model texture pool](src/components/journey/shared-model-textures.ts) is staged for shared external textures, but the runtime URLs must not be switched until both generated profiles and every referenced image are present and verified. Geometry, static poses, UVs and hand/head anchors must remain unchanged.
- The MMA audience uses **242 real 3D foreground fans** and **972 distant 2D fans**. People within seven metres of the full camera route, including the approach and exit, stay 3D. The background uses four instanced cutout batches (1,944 triangles), rather than thousands of animated objects.
...
- [World residency](src/components/journey/world-residency.ts) keeps at most two heavy worlds mounted. Bermel now unloads inside the MMA entrance instead of remaining resident through the ring close-ups; Harar loads during the arena exit. Small differences between entering and leaving thresholds prevent repeated loading when scrolling back and forth at a boundary. Pausing animation saves processing; unloading worlds and releasing their assets saves memory. The camera route itself is unchanged.
- The arena lights remain mounted independently of its models, avoiding a change in every lit material's shader when entering the crowd. Shadows remain on the cage and performers; the instanced audiences no longer enter the cage's shadow pass. Their textured 3D geometry and colored clothing are unchanged.
...
The shared-human optimization is not enabled yet. Generate and verify its assets before switching runtime URLs, and regenerate them after rebuilding either original human profile:
...
The generated shared-texture attribution report must record the input hashes, material mapping and unchanged geometry checks. Sharing the ten images is expected to replace two 37.33 MiB decoded RGBA texture sets with one (including mipmaps); this saving is not active yet and is not a measurement of total browser RAM.
...
- [Bermel's main stage](src/components/journey/scenes/dj-stage.tsx) retains its original Bermel LED artwork and DJ inside the [red-draped hall](src/components/journey/scenes/bermel-hall.tsx). The [band](src/components/journey/scenes/bermel-band.tsx) has a guitarist, front-centre vocalist, keyboardist and standing percussionist, with a 61-key keyboard, congas, microphone stand, monitors, amp and floor cables. The former off-axis band is removed rather than duplicating performers; its fantasy-creature drummer is not used.
- The guitarist uses [guitarist.glb](public/models/guitarist.glb), **Animated Musical Trem Playing Guitar Loop** by [LasquetiSpice](https://sketchfab.com/LasquetiSpice), CC BY 4.0. It is grounded and scaled from a performed pose rather than a T-pose. The animated-model helper uses the shared movement/proximity controller; an explicit `paused` prop can keep an individual actor still.
